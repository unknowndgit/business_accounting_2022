 ...
 {
   $bill_id=$_REQUEST['action_view'];
   $bills_details=$db->get_row('bills',array('id'=>$bill_id));
   $customer_detail=$db->get_row('contacts',array('id'=>$bills_details['supplier_id']));
  // $user=$db->get_row('users',array('user_id'=>$_SESSION['user_id']));

   $project=unserialize($bills_details['project_id']);

   $item=unserialize($bills_details['item_id']);
   $item_price=unserialize($bills_details['item_price']);
   //$item_description=unserialize($bills_details['item_description']);

   //print_r($item_description);
   $item_qty=unserialize($bills_details['item_qty']);
   $item_taxcode=unserialize($bills_details['taxcode']);
   $item_tax=unserialize($bills_details['tax']);
   $item_amount=unserialize($bills_details['amount']);
}
            ...
            <div class="col-lg-12">
              <div class="row invoice-header">
                ...
                <div class="col-md-6 text-right">
                  <h2>
                    <?php echo "#".$bills_details['bill_number'];?>
                  </h2>
                 <p>
                  Bill date: <?php echo $bills_details['bill_date'];?>
                  </p>
                    <p>
                   Reference code :
                   <?php
                   if($bills_details['reference_code']==""){
                       echo "N/A";
                   }
                   else
                   {
                      echo $bills_details['reference_code'];
                   }
                   ?>
                  </p>
                    <p>
                   Due date :
                    <?php
                   if($bills_details['due_date']==""){
                       echo "N/A";
                   }
                   else
                   {
                      echo $bills_details['due_date'];
                   }
                   ?>

                  </p>
                  <p>
                   	<strong>Payment Status :</strong> <?php echo ucfirst($bills_details['payment_status']);?>
                  </p>
                  </div>
              </div>
            </div>
                ...
                <div class="widget-content padded clearfix">
                  <table class="table table-striped invoice-table">
                    ...
                    <tr>
                      ...
                      <th width="10%">Qty</th>
                  <?php if ($bills_details['gst_registered']!='no'){?>
                    ...
                    <td></td>
                     ...
                     <tfoot>
                      <tr>
                        ...
                        <td >
                          <?php echo CURRENCY." ".number_format($bills_details['subtotal'],2,'.',',');?>
                        </td>
                      </tr>
                  <?php if ($bills_details['gst_registered']!='no'){?>
                      <tr>
                        ...
                        <td>
                        <?php echo CURRENCY." ".number_format($bills_details['total_tax'],2,'.',',');?>
                        </td>
                      </tr>
                  <?php }?>
                     <tr>
                        ...
                        <td>
                          <h4 class="text-primary">
                          <?php echo CURRENCY." ".number_format($bills_details['total_amount'],2,'.',',');?>
                          </h4>
                        </td>
                      </tr>
                    </tfoot>
