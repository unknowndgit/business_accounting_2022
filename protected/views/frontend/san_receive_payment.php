<?php
if (isset($_REQUEST['san_id']))
{
	$san_id=$_REQUEST['san_id'];
	$san_details=$db->get_row('supplier_adjustment_notes',array('id'=>$san_id));
	$customer_detail=$db->get_row('contacts',array('id'=>$san_details['supplier_id']));
	$san_paid_amount=$san_details['paid_amount'];
}
    ...
    elseif ($_POST['how_much_allocate']!= $remaining_amount){
    	$display_msg= '<div class="alert alert-danger"><i class="lnr lnr-smile"></i>
            ...
            $insert=$db->insert('receive_money',array('contact_id'=>$contact_id,
			                     ...
			                     'receive_money_for'=>'san',
			                     'receive_money_for_id'=>$san_id,
			...
			{
			    /********************* entry in activity***************************/
			    $adjustment_number=$db->get_var('supplier_adjustment_notes',array('id'=>$san_id),'adjustment_number');
			    $event="Receive payment of supplier adjustment note  (" . $adjustment_number . ")";
			  ...
			  /*to update current balance of the seleted account*/
				...
				$inv_amount_paid=$san_paid_amount + $how_much_allocate;

		          $remaing_update=$db->update('supplier_adjustment_notes',array('balance_remaining'=>$inv_remain_payment,
							                                   'paid_amount'=>$inv_amount_paid),array('id'=>$san_id));

					if ($remaing_update)
					{
						$san_paid_amount=$db->get_var('supplier_adjustment_notes',array('id'=>$san_id),'paid_amount');
						$inv_total_amount=$db->get_var('supplier_adjustment_notes',array('id'=>$san_id),'total_amount');
						if($san_paid_amount == $inv_total_amount)
						{
							$payment_status_update=$db->update('supplier_adjustment_notes',array('payment_status'=>'paid', 'status'=>'paid'),array('id'=>$san_id));
							...
							if ($payment_status_update)
							{
							    ...
							    $summary="generate from Receipts for supplier adjusment notes";
...
/***************************Entry in tax calculation table start*************************************/
	if ($san_details['gst_registered']=="yes"){
	...
	<div class=" padded" >
    	...
    	<div class="widget-container fluid-height">
        	<div class="widget-content padded">
                   ...
                   <a href="<?php echo $link->link('san_make_payment',user);?>" class="btn btn-default pull-right">Cancel</a>
                   ...
                   <div class="widget-content padded">
                     ...
                     <div class="col-lg-6">
                       <div class="form-group">
                           ...
                           <div class="col-md-7">
                           		<h3>#<?php echo $san_details['adjustment_number'];?></h3>
                           </div>
                        </div>
                      <div class="form-group">
                           <label class="control-label col-md-3">Contact<font color="red">*</font></label>
                           <div class="col-md-7">
                           		<?php $all_contact_name=$db->get_row('contacts',array('id'=>$san_details['supplier_id'], 'visibility_status'=>'active')); ?>
								...
								<input class="form-control" placeholder="" type="hidden" name="contact_id" value="<?php echo $all_contact_name['id'];?>">
                           		...
                           		<!-- <select class="form-control" name="contact_id" id=contact_id required readonly>
                                          <?php $all_contact_name=$db->get_row('contacts',array('id'=>$san_details['customer_id'], 'visibility_status'=>'active'));
  ...
  <div class="form-group">
                     ...
                     <div class="col-md-6">
                        ...
                        <div class="form-group">
                         ...
                         <div class="col-md-7">
                              <input class="form-control" placeholder="$0" type="text" name="remaining_amount" id="amount" value="<?php echo $san_details['balance_remaining'];?>" readonly>
                           </div>
                           </div>
                        </div>
                           <div class="form-group">
   ...
   <table class="table table-bordered table-striped" id="dataTable112">
					                  ...
					                  <tbody>
					                    ...
					                    <tr>
					                      ...
					                      <td><?php echo $san_details['adjustment_date'];?></td>
					                      <td><?php echo $san_details['adjustment_number'];?>
					                      		<input class="form-control" type="hidden" name="invoice_id" value="<?php echo $san_details['id'];?>">
					                      </td>
					                      <td>Invoice</td>
					                      <td><?php echo CURRENCY . " " . $san_details['total_amount'];?></td>
					                      <td><?php echo CURRENCY . " " . $san_details['balance_remaining'];?></td>
                                          <td><input class="form-control" type="text" name="how_much_allocate" value="<?php echo $san_details['balance_remaining'];?>"></td>
					                ...
					                <hr>
					                    <h5><strong>Other Supplier adjustment notes related to <?php echo $customer_detail['display_name']?></strong></h5>
...
$sql="SELECT* FROM `supplier_adjustment_notes` WHERE `supplier_id`='$san_details[supplier_id]' AND (`status`='approved' OR `status`='unpaid') AND `id`!='$san_id'";
					                    ...
					                    <tr>
                                          ...
                                          <td>
                                            <?php if ($ob['payment_status']=="paid"){?>
                                                ...
                                                <a
															href="<?php echo $link->link('san_receive_payment',user,'&san_id='.$ob['id']);?>"><i
																class="fa fa-plus"></i>Issue Refund</a>
                                                <?php }?>
                                         </td>
